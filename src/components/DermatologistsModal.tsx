...
import { DermatologistsPage } from "./DermatologistsPage";

interface DermatologistsModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const DermatologistsModal: React.FC<DermatologistsModalProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-4xl w-full h-[85vh] overflow-y-auto p-4 sm:p-6 shadow-2xl space-y-4 relative">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Location-based discovery & verified skin specialist directory
              </p>
            </div>
          </div>
        ...
        <DermatologistsPage isOpen={isOpen} />
      </div>
    </div>
  );
};
